...
Properties already enforced by the current foundation:
...
- prepare and commit queues honor `IMMEDIATE / VISIBLE / NEARBY / PREFETCH / BACKGROUND`;
- an in-flight prefetch can be promoted without duplicate loading;
- commit processing stops when its time budget is consumed;
- VAFS uses canonical path checks and rejects directory/symlink escape;
- SHA-256 content objects are streamed to disk and manifest-routed by logical path.

Later BC resource milestones will add server-assisted hints, decoded/compiled model caches, reference accounting, memory/VRAM budgeting, and Forge/OpenGL upload adapters on this contract.

## Network foundation

The Forge-independent network layer now defines BC protocol version negotiation and capability negotiation. Protocol major versions must match; the lower minor version is selected. Required capabilities must exist on both peers. `BoundedBinaryCodec` limits VarInt/VarLong lengths and validates remote UTF-8 byte lengths before allocating buffers. Forge/FML will later be used only as the transport bridge for these BC protocol frames.
